...
public class TestMain {
	 ...
	 * @param frameRate 帧频率（默认：3）  					//播放频率-一秒几帧
	 * @param margin 每截取一次跳过多少帧（默认：3） //截取步进
	...
	public static void buildGif(String videofile,int startFrame,int frameCount,Integer frameRate,Integer margin) throws IOException {
	    ...
	    ff.start();
		System.out.println("视频总帧数："+ff.getLengthInFrames());
		System.out.println("视频长度："+ff.getLengthInTime() / (1000 * 1000f));
	    try {
